							   ...
							   RandomAccessIterator last, Compare compare)
{
	...
	merge_insertion_sort_impl(make_group_iterator(first, 2),
							  make_group_iterator(end, 2), compare);
	...
	for (int k = 0;; ++k)
	{
		...
		if (dist >
			static_cast<size_type>(std::distance(current_pend, std::end(pend))))
			break;
		...
		do
		{
			...
			auto insertion_point =
				std::upper_bound(std::begin(chain), *pe, *it,
	...
	{
		auto insertion_point =
			std::upper_bound(std::begin(chain), *current_pend, *current_it,
							 [=](const auto &lhs, const auto &rhs)
							 { return compare(lhs, *rhs); });
